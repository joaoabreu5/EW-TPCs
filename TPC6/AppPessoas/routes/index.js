...
router.get('/', function(req, res) {
  Pessoa.list()
    ...
    .catch(erro => res.status(520).json({erro: erro, mensagem: "Não consegui obter a lista de pessoa."}))
});
...
router.post('/pessoas/lista', (req, res) => {
  Pessoa.addListaPessoas(req.body)
    .then(dados => res.status(201).json(dados))
    .catch(erro => res.status(522).json({erro: erro, mensagem: "Não consegui inserir a lista de pessoas."}))
})
